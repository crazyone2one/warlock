...
import cn.master.horde.common.util.WFileUtils;
...
public class LocalFileRepository implements FileRepository {
    ...
    public String saveFile(MultipartFile multipartFile, FileRequest request) throws Exception {
        ...
        WFileUtils.validateFileName(request.getFolder(), request.getFileName());
    ...
    public void copyFile(FileCopyRequest request) {
        throw new BizException("Not support copy file");
    }
    ...
    private String getFilePath(FileRequest request) {
        WFileUtils.validateFileName(request.getFolder(), request.getFileName());
        return StringUtils.join(getFileDir(request), File.separator, request.getFileName());
    }
